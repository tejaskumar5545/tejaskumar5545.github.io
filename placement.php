...
if ($search !== '') {
    ...
    $where = "WHERE title LIKE '%$search_safe%' OR company LIKE '%$search_safe%' OR role LIKE '%$search_safe%'";
}

$placement = mysqli_query($conn, "SELECT * FROM placement $where ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Placement Corner - ClassroomX</title>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:8px;color:var(--white);">Placement Corner</h2>
    <p style="color:var(--gray-700);font-size:15px;margin-bottom:28px;">Latest campus drives, job openings and internship opportunities for students.</p>

    <form method="GET" action="placement.php">
        <div class="filter-bar">
            <input type="text" name="search" placeholder="Search by company, role or title..." value="<?php echo htmlspecialchars($search); ?>">
            <button type="submit" class="btn-filter">Search</button>
            <?php if ($search !== ''): ?>
                <a href="placement.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </div>
    </form>

    <?php if ($placement && mysqli_num_rows($placement) > 0): ?>
        <p style="color:var(--gray-700);font-size:14px;margin-bottom:16px;">Showing <?php echo mysqli_num_rows($placement); ?> opening(s)</p>
        <div class="cards">
            <?php while ($row = mysqli_fetch_assoc($placement)): ?>
                <div class="card placement-card">
                    <span class="card-badge badge-branch"><?php echo htmlspecialchars($row['company']); ?></span>
                    <?php if (!empty($row['package'])): ?>
                        <span class="card-badge badge-sem"><?php echo htmlspecialchars($row['package']); ?></span>
                    <?php endif; ?>
                    <h3><?php echo htmlspecialchars($row['title']); ?></h3>
                    <p class="card-desc">
                        <?php if (!empty($row['role'])): ?>Role: <?php echo htmlspecialchars($row['role']); ?><br><?php endif; ?>
                        <?php echo nl2br(htmlspecialchars($row['description'])); ?>
                    </p>
                    <div class="card-meta">
                        <span class="card-date">Posted: <?php echo date('d M Y', strtotime($row['created_at'])); ?></span>
                        <?php if (!empty($row['link'])): ?>
                            <a href="<?php echo htmlspecialchars($row['link']); ?>" class="btn btn-primary btn-sm" target="_blank" rel="noopener">Apply Now</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endwhile; ?>
    ...
    <?php else: ?>
        <div class="empty-state">
            ...
            <p>New drives will be posted here as they are announced.</p>
        </div>
    <?php endif; ?>
</div>
