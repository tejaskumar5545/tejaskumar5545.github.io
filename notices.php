...
$categories = ['General','Exam','Admission','Fee','Event','Sports','Holiday','Emergency'];
$filter_cat = isset($_GET['category']) ? $_GET['category'] : '';
$search = isset($_GET['search']) ? trim($_GET['search']) : '';

$where = [];
if ($filter_cat !== '') {
    $where[] = "category = '" . mysqli_real_escape_string($conn, $filter_cat) . "'";
}
if ($search !== '') {
...
$notices = mysqli_query($conn, "SELECT * FROM notices $where_sql ORDER BY is_important DESC, created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Notices - ClassroomX</title>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:8px;color:var(--white);">College Notices</h2>
    <p style="color:var(--gray-700);font-size:15px;margin-bottom:28px;">Stay updated with the latest notices and announcements from the college.</p>
    ...
    <form method="GET" action="notices.php">
        <div class="filter-bar">
            ...
            <?php if ($filter_cat !== '' || $search !== ''): ?>
                <a href="notices.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </div>
    </form>
    ...
    <?php if ($notices && mysqli_num_rows($notices) > 0): ?>
        <div style="display:flex;flex-direction:column;gap:16px;">
            <?php while ($row = mysqli_fetch_assoc($notices)): ?>
                <div class="notice-card <?php echo $row['is_important'] ? 'notice-important' : ''; ?>">
                    ...
                    <?php if (!empty($row['content'])): ?>
                        <p class="notice-content"><?php echo nl2br(htmlspecialchars($row['content'])); ?></p>
                    <?php endif; ?>
                    <div class="notice-date">Posted on <?php echo date('d M Y, h:i A', strtotime($row['created_at'])); ?></div>
                </div>
            <?php endwhile; ?>
        </div>
    <?php else: ?>
