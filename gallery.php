...
$where = '';
if ($filter_cat !== '') {
    $where = "WHERE category = '" . mysqli_real_escape_string($conn, $filter_cat) . "'";
}
$images = mysqli_query($conn, "SELECT * FROM gallery_images $where ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Gallery - ClassroomX</title>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:8px;color:var(--white);">Campus Gallery</h2>
    <p style="color:var(--gray-700);font-size:15px;margin-bottom:28px;">Moments from our campus, events, and student life.</p>

    <div class="filter-bar">
        <form method="GET" action="gallery.php" style="display:flex;gap:12px;flex-wrap:wrap;">
            ...
            <?php if ($filter_cat !== ''): ?>
                <a href="gallery.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </form>
    </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($images)): ?>
                <div class="gallery-item">
                    ...
                    <div class="gallery-overlay">
                        ...
                        <span style="font-size:11px;opacity:0.8;"><?php echo htmlspecialchars($row['category']); ?></span>
                    </div>
                </div>
            <?php endwhile; ?>
        </div>
