...
$err = '';
$old = [
    'title' => '', 'description' => '', 'duration_minutes' => 30, 'total_marks' => 0,
    'pass_percentage' => 40, 'semester' => '', 'branch' => '', 'instructions' => '', 'is_active' => 1
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($old['title']) || $old['duration_minutes'] <= 0) {
        ...
        $query = "INSERT INTO exams (title, description, duration_minutes, total_marks, pass_percentage, semester, branch, instructions, is_active)
                  VALUES ('$title', '$description', {$old['duration_minutes']}, {$old['total_marks']}, {$old['pass_percentage']}, '$semester', '$branch', '$instructions', {$old['is_active']})";
        if (mysqli_query($conn, $query)) {
            $new_id = mysqli_insert_id($conn);
            header("Location: admin_exam_questions.php?id=$new_id&msg=created");
            exit;
        } else {
            $err = 'Failed to create exam. Please try again.';
        }
    }
}
?>
...
<head>
    ...
    <title>Add Exam - ClassroomX</title>
    ...
    <div class="upload-box">
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" name="title" placeholder="e.g. Data Structures - Mid Term Test" value="<?php echo htmlspecialchars($old['title']); ?>" required>
            </div>
            <div class="form-group">
                <label>Description</label>
                <textarea name="description" rows="3" placeholder="Short description of the exam"><?php echo htmlspecialchars($old['description']); ?></textarea>
            </div>
            <div class="form-row">
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea name="instructions" rows="4" placeholder="Instructions shown to students before starting the exam"><?php echo htmlspecialchars($old['instructions']); ?></textarea>
            </div>
            <div class="form-group">
