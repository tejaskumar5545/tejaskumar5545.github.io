...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($act==='add'){
        $stmt=$conn->prepare("INSERT INTO placement(title,company,description,role,package,link) VALUES(?,?,?,?,?,?)");
        $stmt->bind_param("ssssss",$title,$company,$description,$role,$package,$link);$stmt->execute();$stmt->close();
        $_SESSION['admin_msg']='Placement resource added!';header("Location: placement.php");exit;
    }elseif($act==='edit'&&$id>0){
        $stmt=$conn->prepare("UPDATE placement SET title=?,company=?,description=?,role=?,package=?,link=? WHERE id=?");
        $stmt->bind_param("ssssssi",$title,$company,$description,$role,$package,$link,$id);$stmt->execute();$stmt->close();
        $_SESSION['admin_msg']='Placement resource updated!';header("Location: placement.php");exit;
    }elseif($act==='delete'&&$id>0){
        $conn->query("DELETE FROM placement WHERE id=$id");$_SESSION['admin_msg']='Deleted!';header("Location: placement.php");exit;
    }
}
$edit_row=null;if($action==='edit'&&$id>0){$r=$conn->query("SELECT * FROM placement WHERE id=$id");if($r&&$r->num_rows>0)$edit_row=$r->fetch_assoc();}
$items=$conn->query("SELECT * FROM placement ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="theme-color" content="#0f172a">
<title>Placement Resources - EngiHub Admin</title>
<style>
...
<div class="nl">Resources</div><a href="coding.php"><span class="ni">&#128187;</span>Coding</a><a href="projects.php"><span class="ni">&#128640;</span>Projects</a><a href="placement.php" class="active"><span class="ni">&#127919;</span>Placement</a>
...
<div class="page-header"><h1><?php echo $action==='add'?'&#127919; Add Placement Resource':'&#9998; Edit Placement Resource';?></h1><a href="placement.php" class="btn btn-secondary">&#8592; Back</a></div>
<div class="card">
...
<div class="form-group"><label>Company</label><input type="text" name="company" value="<?php echo htmlspecialchars($edit_row['company']??'');?>"></div>
<div class="form-group"><label>Role</label><input type="text" name="role" value="<?php echo htmlspecialchars($edit_row['role']??'');?>"></div>
<div class="form-group"><label>Package</label><input type="text" name="package" value="<?php echo htmlspecialchars($edit_row['package']??'');?>" placeholder="e.g. 6-8 LPA"></div>
<div class="form-group"><label>Link</label><input type="url" name="link" value="<?php echo htmlspecialchars($edit_row['link']??'');?>"></div>
...
<div style="margin-top:16px"><button type="submit" class="btn btn-primary">&#10003; <?php echo $action==='add'?'Add Resource':'Update';?></button></div>
</form>
</div>
<?php else:?>
<div class="page-header"><h1>&#127919; Placement Resources</h1><a href="placement.php?action=add" class="btn btn-primary">+ Add Resource</a></div>
<div class="card"><table><thead><tr><th>Title</th><th>Company</th><th>Role</th><th>Package</th><th>Date</th><th>Actions</th></tr></thead><tbody>
<?php if($items&&$items->num_rows>0):while($row=$items->fetch_assoc()):?>
<tr><td style="font-weight:600"><?php echo htmlspecialchars($row['title']);?></td><td><?php echo htmlspecialchars($row['company']??'-');?></td><td><?php echo htmlspecialchars($row['role']??'-');?></td><td><?php echo htmlspecialchars($row['package']??'-');?></td><td><?php echo date('d M Y',strtotime($row['created_at']));?></td>
<td><div style="display:flex;gap:4px"><a href="placement.php?action=edit&id=<?php echo $row['id'];?>" class="btn btn-primary btn-sm">Edit</a><form method="POST" style="display:inline"><input type="hidden" name="act" value="delete"><input type="hidden" name="id" value="<?php echo $row['id'];?>"><button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Delete?')">Delete</button></form></div></td></tr>
<?php endwhile;else:?><tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:30px">No placement resources yet.</td></tr><?php endif;?>
</tbody></table></div>
<?php endif;?>
</div>
</body>
