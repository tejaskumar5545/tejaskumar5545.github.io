        ...
        if (empty($msg)) {
            if ($act === 'add') {
                if (empty($pdf_name)) { $msg = "Please select a PDF!"; }
                    ...
                    $stmt = $conn->prepare("INSERT INTO practicals (title, subject, branch, semester, pdf_file) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssss", $title, $subject, $branch, $semester, $pdf_name);
                    $stmt->execute(); $stmt->close();
                    $_SESSION['admin_msg'] = "Practical added!"; header("Location: practicals.php"); exit;
                }
            } elseif ($act === 'edit' && $id > 0) {
                if ($pdf_name) {
                    $stmt = $conn->prepare("UPDATE practicals SET title=?, subject=?, branch=?, semester=?, pdf_file=? WHERE id=?");
                    $stmt->bind_param("sssssi", $title, $subject, $branch, $semester, $pdf_name, $id);
                } else {
                    $stmt = $conn->prepare("UPDATE practicals SET title=?, subject=?, branch=?, semester=? WHERE id=?");
                    $stmt->bind_param("ssssi", $title, $subject, $branch, $semester, $id);
                }
                $stmt->execute(); $stmt->close();
                $_SESSION['admin_msg'] = "Practical updated!"; header("Location: practicals.php"); exit;
            }
        }
    } elseif ($act === 'delete' && $id > 0) {
        $r = $conn->query("SELECT pdf_file FROM practicals WHERE id=$id");
        if ($r && $r->num_rows > 0) { $row = $r->fetch_assoc(); @unlink('../uploads/practicals/' . $row['pdf_file']); }
        $conn->query("DELETE FROM practicals WHERE id=$id");
        $_SESSION['admin_msg'] = "Practical deleted!"; header("Location: practicals.php"); exit;
    }
}
$edit_row = null;
if ($action === 'edit' && $id > 0) { $r = $conn->query("SELECT * FROM practicals WHERE id=$id"); if ($r && $r->num_rows > 0) $edit_row = $r->fetch_assoc(); }
$items = $conn->query("SELECT * FROM practicals ORDER BY upload_date DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="theme-color" content="#1e3a5f">
    <title>Manage Practicals - EngiHub Admin</title>
    <style>
...
<div class="sidebar">
    ...
    <nav>
        <a href="index.php">&#127968; Dashboard</a><a href="notes.php">&#128218; Manage Notes</a><a href="syllabus.php">&#128218; Manage Syllabus</a><a href="pyq.php">&#128196; Manage PYQ</a><a href="practicals.php" class="active">&#128295; Manage Practicals</a><a href="students.php">&#127891; Students</a><a href="notices.php">&#128227; Notices</a>
    </nav>
...
<div class="main">
    ...
    <?php if ($action === 'add' || $action === 'edit'): ?>
        <div class="page-header"><h1><?php echo $edit_row ? 'Edit Practical' : 'Add Practical'; ?></h1><a href="practicals.php" class="btn btn-primary">&#8592; Back</a></div>
        <div class="card"><form method="POST" action="practicals.php" enctype="multipart/form-data">
    ...
    <?php else: ?>
        <div class="page-header"><h1>Manage Practicals</h1><a href="practicals.php?action=add" class="btn btn-primary">+ Add Practical</a></div>
        ...
        <?php if ($items && $items->num_rows > 0): while ($row = $items->fetch_assoc()): ?>
            ...
            <td><div class="actions-cell"><a href="practicals.php?action=edit&id=<?php echo $row['id'];?>" class="btn btn-primary btn-sm">Edit</a><a href="practicals.php?action=delete&id=<?php echo $row['id'];?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete?')">Delete</a></div></td></tr>
        <?php endwhile; else: ?><tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:30px">No practicals yet.</td></tr><?php endif; ?>
        </tbody></table></div>
    <?php endif; ?>
</div></body></html>
