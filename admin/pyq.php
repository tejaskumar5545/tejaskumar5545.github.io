        ...
        if (empty($msg)) {
            if ($act === 'add') {
                if (empty($pdf_name)) { $msg = "Please select a PDF file!"; }
                    ...
                    $stmt = $conn->prepare("INSERT INTO pyq (title, subject, branch, semester, year, pdf_file) VALUES (?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("ssssss", $title, $subject, $branch, $semester, $year, $pdf_name);
                    $stmt->execute(); $stmt->close();
                    $_SESSION['admin_msg'] = "PYQ added!"; header("Location: pyq.php"); exit;
                }
            } elseif ($act === 'edit' && $id > 0) {
                if ($pdf_name) {
                    $stmt = $conn->prepare("UPDATE pyq SET title=?, subject=?, branch=?, semester=?, year=?, pdf_file=? WHERE id=?");
                    $stmt->bind_param("ssssssi", $title, $subject, $branch, $semester, $year, $pdf_name, $id);
                } else {
                    $stmt = $conn->prepare("UPDATE pyq SET title=?, subject=?, branch=?, semester=?, year=? WHERE id=?");
                    $stmt->bind_param("sssssi", $title, $subject, $branch, $semester, $year, $id);
                }
                $stmt->execute(); $stmt->close();
                $_SESSION['admin_msg'] = "PYQ updated!"; header("Location: pyq.php"); exit;
            }
        }
    } elseif ($act === 'delete' && $id > 0) {
        $r = $conn->query("SELECT pdf_file FROM pyq WHERE id=$id");
        if ($r && $r->num_rows > 0) { $row = $r->fetch_assoc(); @unlink('../uploads/pyq/' . $row['pdf_file']); }
        $conn->query("DELETE FROM pyq WHERE id=$id");
        $_SESSION['admin_msg'] = "PYQ deleted!"; header("Location: pyq.php"); exit;
    }
}
$edit_row = null;
if ($action === 'edit' && $id > 0) { $r = $conn->query("SELECT * FROM pyq WHERE id=$id"); if ($r && $r->num_rows > 0) $edit_row = $r->fetch_assoc(); }
$items = $conn->query("SELECT * FROM pyq ORDER BY year DESC, upload_date DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="theme-color" content="#1e3a5f">
    <title>Manage PYQ - EngiHub Admin</title>
    <style>
...
<div class="main">
    ...
    <?php if ($action === 'add' || $action === 'edit'): ?>
        <div class="page-header"><h1><?php echo $edit_row ? 'Edit PYQ' : 'Add New PYQ'; ?></h1><a href="pyq.php" class="btn btn-primary">&#8592; Back</a></div>
        <div class="card"><form method="POST" action="pyq.php" enctype="multipart/form-data">
            ...
            <div class="form-grid">
                ...
                <div class="form-group"><label>Subject</label><input type="text" name="subject" value="<?php echo htmlspecialchars($edit_row['subject'] ?? ''); ?>" placeholder="e.g. Data Structures"></div>
                ...
                <div class="form-group"><label>Year</label><select name="year" required><option value="">Select</option><?php for($y=2025;$y>=2015;$y--):?><option value="<?php echo $y;?>" <?php if(($edit_row['year']??'')===(string)$y)echo'selected';?>><?php echo $y;?></option><?php endfor;?></select></div>
                <div class="form-group"><label>PDF File</label><input type="file" name="pdf_file" accept=".pdf" <?php echo $edit_row ? '' : 'required'; ?>><?php if($edit_row):?><p style="font-size:12px;color:#6b7280;margin-top:4px">Current: <?php echo $edit_row['pdf_file']; ?></p><?php endif;?></div>
            </div>
            <button type="submit" class="btn btn-primary"><?php echo $edit_row ? 'Update' : 'Upload'; ?></button>
        </form></div>
    <?php else: ?>
        <div class="page-header"><h1>Manage PYQ</h1><a href="pyq.php?action=add" class="btn btn-primary">+ Add PYQ</a></div>
        <div class="card"><table><thead><tr><th>Title</th><th>Subject</th><th>Branch</th><th>Sem</th><th>Year</th><th>Actions</th></tr></thead><tbody>
        <?php if ($items && $items->num_rows > 0): while ($row = $items->fetch_assoc()): ?>
            <tr><td><?php echo htmlspecialchars($row['title']); ?></td><td><?php echo htmlspecialchars($row['subject'] ?? '-'); ?></td><td><?php echo htmlspecialchars($row['branch']); ?></td><td><?php echo htmlspecialchars($row['semester']); ?></td><td><?php echo htmlspecialchars($row['year']); ?></td>
            <td><div class="actions-cell"><a href="pyq.php?action=edit&id=<?php echo $row['id'];?>" class="btn btn-primary btn-sm">Edit</a><a href="pyq.php?action=delete&id=<?php echo $row['id'];?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete?')">Delete</a></div></td></tr>
        <?php endwhile; else: ?><tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:30px">No PYQ yet.</td></tr><?php endif; ?>
        </tbody></table></div>
    <?php endif; ?>
</div></body></html>
