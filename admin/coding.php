...
$conn->query("CREATE TABLE IF NOT EXISTS `coding`(`id` int(11) NOT NULL AUTO_INCREMENT,`title` varchar(255) NOT NULL,`description` text DEFAULT NULL,`language` varchar(50) DEFAULT NULL,`difficulty` varchar(20) DEFAULT 'Easy',`code` text DEFAULT NULL,`created_at` timestamp NOT NULL DEFAULT current_timestamp(),PRIMARY KEY(`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci");
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $difficulty=sanitize($conn,$_POST['difficulty']??'Easy');
    $code=$_POST['code']??'';
    if($act==='add'){
        $stmt=$conn->prepare("INSERT INTO coding(title,description,language,difficulty,code) VALUES(?,?,?,?,?)");
        $stmt->bind_param("sssss",$title,$description,$language,$difficulty,$code);$stmt->execute();$stmt->close();
        $_SESSION['admin_msg']='Coding resource added!';header("Location: coding.php");exit;
    }elseif($act==='edit'&&$id>0){
        $stmt=$conn->prepare("UPDATE coding SET title=?,description=?,language=?,difficulty=?,code=? WHERE id=?");
        $stmt->bind_param("sssssi",$title,$description,$language,$difficulty,$code,$id);$stmt->execute();$stmt->close();
        $_SESSION['admin_msg']='Coding resource updated!';header("Location: coding.php");exit;
    }elseif($act==='delete'&&$id>0){
        $conn->query("DELETE FROM coding WHERE id=$id");
        $_SESSION['admin_msg']='Coding resource deleted!';header("Location: coding.php");exit;
    }
}
$edit_row=null;
if($action==='edit'&&$id>0){$r=$conn->query("SELECT * FROM coding WHERE id=$id");if($r&&$r->num_rows>0)$edit_row=$r->fetch_assoc();}
$items=$conn->query("SELECT * FROM coding ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="theme-color" content="#0f172a">
<title>Coding Resources - EngiHub Admin</title>
<style>
...
<div class="nl">Resources</div><a href="coding.php" class="active"><span class="ni">&#128187;</span>Coding</a><a href="projects.php"><span class="ni">&#128640;</span>Projects</a><a href="placement.php"><span class="ni">&#127919;</span>Placement</a>
...
<div class="page-header"><h1><?php echo $action==='add'?'&#128187; Add Coding Resource':'&#9998; Edit Coding Resource';?></h1><a href="coding.php" class="btn btn-secondary">&#8592; Back</a></div>
<div class="card">
<form method="POST"><input type="hidden" name="act" value="<?php echo $action;?>"><input type="hidden" name="id" value="<?php echo $id;?>">
<div class="form-grid">
...
<div class="form-group"><label>Language</label><select name="language"><option value="C" <?php echo ($edit_row['language']??'')==='C'?'selected':'';?>>C</option><option value="C++" <?php echo ($edit_row['language']??'')==='C++'?'selected':'';?>>C++</option><option value="Python" <?php echo ($edit_row['language']??'')==='Python'?'selected':'';?>>Python</option><option value="Java" <?php echo ($edit_row['language']??'')==='Java'?'selected':'';?>>Java</option><option value="JavaScript" <?php echo ($edit_row['language']??'')==='JavaScript'?'selected':'';?>>JavaScript</option><option value="Other" <?php echo ($edit_row['language']??'')==='Other'?'selected':'';?>>Other</option></select></div>
<div class="form-group"><label>Difficulty</label><select name="difficulty"><option value="Easy" <?php echo ($edit_row['difficulty']??'')==='Easy'?'selected':'';?>>Easy</option><option value="Medium" <?php echo ($edit_row['difficulty']??'')==='Medium'?'selected':'';?>>Medium</option><option value="Hard" <?php echo ($edit_row['difficulty']??'')==='Hard'?'selected':'';?>>Hard</option></select></div>
...
<div class="form-group full"><label>Code</label><textarea name="code" style="font-family:'Courier New',monospace;min-height:150px"><?php echo htmlspecialchars($edit_row['code']??'');?></textarea></div>
</div>
<div style="margin-top:16px"><button type="submit" class="btn btn-primary">&#10003; <?php echo $action==='add'?'Add Resource':'Update Resource';?></button></div>
</form>
</div>
<?php else:?>
<div class="page-header"><h1>&#128187; Coding Resources</h1><a href="coding.php?action=add" class="btn btn-primary">+ Add Resource</a></div>
<div class="card"><table><thead><tr><th>Title</th><th>Language</th><th>Difficulty</th><th>Created</th><th>Actions</th></tr></thead><tbody>
<?php if($items&&$items->num_rows>0):while($row=$items->fetch_assoc()):?>
<tr><td style="font-weight:600"><?php echo htmlspecialchars($row['title']);?></td><td><?php echo htmlspecialchars($row['language']);?></td><td><?php echo htmlspecialchars($row['difficulty']);?></td><td><?php echo date('d M Y',strtotime($row['created_at']));?></td>
<td><div style="display:flex;gap:4px"><a href="coding.php?action=edit&id=<?php echo $row['id'];?>" class="btn btn-primary btn-sm">Edit</a><form method="POST" style="display:inline"><input type="hidden" name="act" value="delete"><input type="hidden" name="id" value="<?php echo $row['id'];?>"><button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Delete?')">Delete</button></form></div></td></tr>
<?php endwhile;else:?><tr><td colspan="5" style="text-align:center;color:#9ca3af;padding:30px">No coding resources yet.</td></tr><?php endif;?>
</tbody></table></div>
<?php endif;?>
</div>
</body>
