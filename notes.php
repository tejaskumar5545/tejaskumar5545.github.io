...
$branches_result = mysqli_query($conn, "SELECT DISTINCT branch FROM notes ORDER BY branch");
$branches = [];
...
$notes = mysqli_query($conn, "SELECT * FROM notes $where_sql ORDER BY upload_date DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Browse Notes - EngiHub</title>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:24px;color:var(--white);">Browse All Notes</h2>

    <?php
    $all_branches = ['Computer Engineering','Information Technology','Electronics Engineering','Mechanical Engineering','Civil Engineering','Electrical Engineering','Automobile Engineering'];
    $branch_icons = ['Computer Engineering'=>'&#128187;','Information Technology'=>'&#128187;','Electronics Engineering'=>'&#128225;','Mechanical Engineering'=>'&#9881;','Civil Engineering'=>'&#127959;','Electrical Engineering'=>'&#9889;','Automobile Engineering'=>'&#128663;'];
    $branch_counts = [];
    $cnt_result = mysqli_query($conn, "SELECT branch, COUNT(*) as c FROM notes GROUP BY branch");
    while ($cr = mysqli_fetch_assoc($cnt_result)) {
        $branch_counts[$cr['branch']] = $cr['c'];
    }
    ?>
    <div class="branch-grid" style="margin-bottom:32px;">
        <?php foreach ($all_branches as $b): ?>
            <a href="notes.php?branch=<?php echo urlencode($b); ?>" class="branch-chip <?php echo $filter_branch === $b ? 'active' : ''; ?>">
                <span class="branch-chip-icon"><?php echo $branch_icons[$b]; ?></span>
                <span class="branch-chip-name"><?php echo $b; ?></span>
                <span class="branch-chip-count"><?php echo isset($branch_counts[$b]) ? $branch_counts[$b] : 0; ?></span>
            </a>
        <?php endforeach; ?>
    </div>
    ...
    <form method="GET" action="notes.php">
        <div class="filter-bar">
            ...
            <?php if ($filter_sem !== '' || $filter_branch !== '' || $search !== ''): ?>
                <a href="notes.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </div>
    </form>

    <?php if ($notes && mysqli_num_rows($notes) > 0): ?>
        <p style="color:var(--gray-700);font-size:14px;margin-bottom:16px;">Showing <?php echo mysqli_num_rows($notes); ?> note(s)</p>
        <div class="cards">
