...
$branches_result = mysqli_query($conn, "SELECT DISTINCT branch FROM syllabus ORDER BY branch");
$branches = [];
...
$syllabus = mysqli_query($conn, "SELECT * FROM syllabus $where_sql ORDER BY semester ASC, upload_date DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Syllabus - ClassroomX</title>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:8px;color:var(--white);">Syllabus</h2>
    <p style="color:var(--gray-700);font-size:15px;margin-bottom:28px;">Download the latest syllabus for your branch and semester.</p>

    <form method="GET" action="syllabus.php">
        <div class="filter-bar">
            ...
            <?php if ($filter_sem !== '' || $filter_branch !== '' || $search !== ''): ?>
                <a href="syllabus.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </div>
    </form>

    <?php if ($syllabus && mysqli_num_rows($syllabus) > 0): ?>
        <p style="color:var(--gray-700);font-size:14px;margin-bottom:16px;">Showing <?php echo mysqli_num_rows($syllabus); ?> file(s)</p>
        <div class="cards">
