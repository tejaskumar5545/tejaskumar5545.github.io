...
$branches_result = mysqli_query($conn, "SELECT DISTINCT branch FROM pyq ORDER BY branch");
$branches = [];
while ($r = mysqli_fetch_assoc($branches_result)) {
    $branches[] = $r['branch'];
}
$years_result = mysqli_query($conn, "SELECT DISTINCT year FROM pyq ORDER BY year DESC");
$years = [];
while ($r = mysqli_fetch_assoc($years_result)) {
    $years[] = $r['year'];
}
...
if ($filter_year !== '') {
    $where[] = "year = '" . mysqli_real_escape_string($conn, $filter_year) . "'";
}
if ($search !== '') {
...
$pyq = mysqli_query($conn, "SELECT * FROM pyq $where_sql ORDER BY year DESC, semester ASC, upload_date DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Previous Year Questions - ClassroomX</title>
...
<header>
    ...
    <nav>
        ...
        <a href="pyq.php" class="active">PYQ</a>
...
<div class="container">
    <h2 style="font-size:28px;font-weight:700;margin-bottom:8px;color:var(--white);">Previous Year Questions</h2>
    <p style="color:var(--gray-700);font-size:15px;margin-bottom:28px;">Practice with previous year exam papers for your branch and semester.</p>

    <form method="GET" action="pyq.php">
        <div class="filter-bar">
            ...
            <?php if ($filter_sem !== '' || $filter_branch !== '' || $filter_year !== '' || $search !== ''): ?>
                <a href="pyq.php" class="btn-reset">Clear</a>
            <?php endif; ?>
        </div>
    </form>

    <?php if ($pyq && mysqli_num_rows($pyq) > 0): ?>
        <p style="color:var(--gray-700);font-size:14px;margin-bottom:16px;">Showing <?php echo mysqli_num_rows($pyq); ?> paper(s)</p>
        <div class="cards">
            <?php while ($row = mysqli_fetch_assoc($pyq)): ?>
                <div class="card">
                    ...
                    <p class="card-desc">
                        ...
                        Exam Year: <?php echo htmlspecialchars($row['year']); ?>
                    </p>
                    <div class="card-meta">
